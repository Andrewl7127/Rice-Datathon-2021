...
app.layout = html.Div(children=[
    ...
    html.H4('Created by Andrew Liu, Adhvaith Vijay, Kyle Fang, and Angeline Eow',
...
def viz_bucket(n, bucket):
    ...
    if bucket_q == "Overall":
        result = training.groupby(['StoreNumber', 'dayOfTheYear', 'latitude', 'longitude']).agg(
            {'GrossSoldQuantity': sum}).reset_index()
        dates_of_interest = np.cumsum([31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 29])
        to_map = [{dates_of_interest[i]: i + 1 for i in range(0, 12)}][0]
        result = result[result['dayOfTheYear'].isin(dates_of_interest)]
        result['Month'] = result['dayOfTheYear'].map(to_map)

    else:
        training_group = training[training['3HourBucket'] == bucket_q]
        result = training_group.groupby(['StoreNumber', 'dayOfTheYear', 'latitude', 'longitude']).agg(
            {'GrossSoldQuantity': sum}).reset_index()
        result = result[result['GrossSoldQuantity'] != 0].reset_index()
        dates_of_interest = np.cumsum([31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 25])

        boolean = True
        while boolean:
            for j in [1000, 2000, 3000, 4000]:
                for i in range(len(dates_of_interest)):
                    if dates_of_interest[i] not in list(result['dayOfTheYear'][result['StoreNumber'] == j]):
                        dates_of_interest[i] = dates_of_interest[i] - 1
            boolean = False
